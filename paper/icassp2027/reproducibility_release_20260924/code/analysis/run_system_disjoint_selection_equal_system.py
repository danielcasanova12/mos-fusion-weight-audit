...
This version uses equal-system MSE as the inner selection objective.  Each
system contributes one equally weighted mean squared error, so systems with
many utterances cannot dominate the gate or the selector.
...
def equal_system_mse(y: np.ndarray, prediction: np.ndarray, groups: np.ndarray) -> float:
    """Mean of per-system MSEs, with every system receiving equal weight."""
    ...
    sums = np.bincount(inverse, weights=squared_error)
    counts = np.bincount(inverse)
    return float(np.mean(sums / counts))


def fit_equal_system_simplex(
    predictions: np.ndarray,
    y: np.ndarray,
    groups: np.ndarray,
) -> np.ndarray:
    """Fit a simplex gate for equal-system MSE.

    Multiplying each row by ``sqrt(1/|I_g|)`` turns the weighted least-squares
    objective into an ordinary least-squares objective, up to a constant.  The
    existing deterministic SLSQP/fallback solver can therefore be reused while
    optimizing the requested system-balanced criterion rather than utterance
    MSE.
    """
    groups = np.asarray(groups)
    _, inverse = np.unique(groups, return_inverse=True)
    counts = np.bincount(inverse)
    row_scale = 1.0 / np.sqrt(counts[inverse].astype(np.float64))
    return controls.fit_simplex(
        np.asarray(predictions, dtype=np.float64) * row_scale[:, None],
        np.asarray(y, dtype=np.float64) * row_scale,
    )
...
def coalition_game(
    ...
    for mask in range(1, 1 << N_FEATURES):
        ...
        gate = fit_equal_system_simplex(prediction[:, indices], y, groups)
        coalition_prediction = prediction[:, indices] @ gate
        losses[mask] = equal_system_mse(y, coalition_prediction, groups)
        weights[mask] = gate
...
def run_fold(
    ...
    losses, coalition_weights = coalition_game(inner_prediction, y[outer_train], inner_groups)
    empty_loss = equal_system_mse(y[outer_train], null_prediction, inner_groups)
            ...
            selection_rows.append(
                {
                    ...
                    "inner_equal_system_mse": float(losses[mask]),
                    "inner_utterance_mse": float(
                        np.mean((y[outer_train] - inner_prediction[:, indices] @ gate) ** 2)
                    ),
    ...
    atomic_json(
        out / "folds" / f"{dataset}_fold{fold}_diagnostics.json",
        {
            ...
            "empty_inner_equal_system_mse": empty_loss,
            "full_inner_equal_system_mse": float(losses[FULL_MASK]),
...
def main() -> None:
    ...
    atomic_json(
        out / "protocol.json",
        {
            ...
            "inner_selection_metric": "equal-system MSE",
            "gate_calibration_metric": "equal-system MSE",
            "outer_evaluation_metric": "equal-system MSE",
            "utterance_mse_role": "diagnostic only; never used for selection or gate fitting",
