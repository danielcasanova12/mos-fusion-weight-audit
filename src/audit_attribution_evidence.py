...
The training Shapley percentiles use 30 refitted training-system replicates on
...
def main() -> None:
    ...
    for filename, value, prefix, expected, positive in [
        ...
        ("full_train_bootstrap_shapley.csv", "shapley_mse_utility", "train_shapley", 30, True),
        ("conditional_gate_bootstrap_replicates.csv", "weight", "frozen_expert_gate", 200, False),
    ]:
        path = args.priority / filename
        if path.exists():
    ...
    for column in ["test_shapley_ci95_low", "test_shapley_ci95_high", "test_shapley_bootstrap_n"]:
        if column not in table:
            table[column] = np.nan
    table["train_shapley_uncertainty_scope"] = "training_system_refit_fixed_observed_test_descriptive_30_replicates"
    ...
    notes = ["# Attribution evidence audit", "", "Run `python paper/icassp2027/audit_attribution_evidence.py` with numpy and pandas.", "",
             ...
             "Training gate probabilities use 100 system-bootstrap refits per corpus. Training Shapley summaries use the first 30 full-system refits and complete coalition games per corpus, conditional on the fixed observed test set. The percentile ranges are descriptive stability ranges, not precise 95% confidence intervals. Positive frequency means the observed fraction of those 30 replicates with Shapley > 0; its resolution is 1/30. It is not a posterior probability or a joint training/test confidence statement.", "",
